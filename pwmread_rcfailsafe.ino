...
const int pwmPIN[]={A0,A1,A2,A3}; // an array to identify the PWM input pins (the array can be any length) 
...
//TURNIGY 9x with Frsky X8r  for TANK
//                ELE/CH1  AIL/CH2    RUD/CH3   IDX/CH4  
//int RC_min[4] = { 996,      1008,     992,      992};
int RC_min[4] = { 900,      900,      900,      900};
int RC_mid[4] = { 1504,     1504,     1504,     1504};
int RC_max[4] = { 2020,     2020,     2020,     2020};

// fail safe positions

float RC_failsafe[] = {0.00, 0.00, 1, 0.00};
...
float RC_decode(int CH){
  ...
  int Min;
  if(CH <= size_RC_min) Min = RC_min[CH-1]; else Min = 900;
  
  int Mid;
  if(CH <= size_RC_mid) Mid = RC_mid[CH-1]; else Mid = 1500;
  
  int Max;
  if(CH <= size_RC_max) Max = RC_max[CH-1]; else Max = 2020;
...
int calibrate(float Rx, int Min, int Mid, int Max){
   ...
   if (Rx >= Mid)
   {
    calibrated = map(Rx, Mid, Max, 1500, 2000);  
   }
   else if (Rx < Mid)
   {
    calibrated = map(Rx, Min, Mid, 1000, 1500); // map from 0% to -100% in the other direction
   }

   if( calibrated > (1500 - deadBand) & calibrated < (1500 + deadBand) ){
      calibrated = 1500; 
   }
